...
if __name__ == "__main__":
    ...
    for i in range(2002, 2019):
        survivalrates[str(i)] = pd.read_excel(
            "data/business-demographics.xlsx",
            sheet_name=str(i) + " Survival Rates",
            skiprows=1,
        )

    # Remove 'Births' and the columns with survival rates in numbers
    for i in range(2002, 2019):
        survivalrates[str(i)].drop(
            survivalrates[str(i)].columns[[2, 3, 5, 7, 9, 11]], axis=1, inplace=True
        )

    # Rename columns
    for i in range(2002, 2019):
        survivalrates[str(i)].rename(
            columns={
                "Per cent": "1 Year Survival in %",
                "Per cent.1": "2 Year Survival in %",
                "Per cent.2": "3 Year Survival in %",
                "Per cent.3": "4 Year Survival in %",
                "Per cent.4": "5 Year Survival in %",
            },
            inplace=True,
        )
    ...
    for i in range(2002, 2019):
        survivalrates[str(i)].drop(
            columns=survivalrates[str(i)].columns[(survivalrates[str(i)] == ":").any()],
            inplace=True,
        )
    ...
    activeenterprises = pd.read_excel(
        "data/business-demographics.xlsx", sheet_name="Active Enterprises by year"
    )
    ...
    # Read the sheets and skip a row
    enterprisedeaths = pd.read_excel(
        "data/business-demographics.xlsx",
        sheet_name="Enterprise deaths by year",
        skiprows=1,
    )
    ...
    with pd.ExcelWriter("data/prepared_data1.xlsx") as writer:
        ...
        for i in range(2002, 2019):
            survivalrates[str(i)].to_excel(
                writer, sheet_name=str(i) + " Survival Rates"
            )
